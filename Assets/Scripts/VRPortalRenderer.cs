...
public class VRPortalRenderer: MonoBehaviour/*, IPlayerInputHandler*/{
	...
	[SerializeField] LayerMask cameraViewMask = 1 << 1 | 1 << 10;
	...
	*/
	...
	// ryan functioon (get visibility of object)

	public bool IsPosRendered(Vector3 pos) {
		Vector3 v = renderCam.WorldToViewportPoint(pos);
		float x = v.z;
		if (x >= 1.0f) {
			return true;
		}
